...
@IBDesignable public class DCRadialGraph: UIView {
    ...
    func animate(){
        ...
        let startAngle = clockwise ? arcOffset : segmentEndAngle + arcOffset
        let endAngle = clockwise ? segmentEndAngle + arcOffset : arcOffset
    ...
    func addAutoLabel(){
        ...
        let sublabel = UILabel(frame: CGRectMake(label.bounds.origin.x, label.bounds.height - label.font.lineHeight/6, label.frame.width, label.font.lineHeight/6))
    ...
    private func updateText(){
        let percent = Int(Float(counter)/Float(totalSegments) * 100)
        label.text = "\(percent)"
    }
}